...
function noteSelectFunction(clicked) {
	...
	switch (clicked) {
	  ...
	  case "advisingCurrentWaterOffEmergency":
	  	blanksArray = ["Name?", "Address?", "Burst Street Name?", "Job Number?", "ETR?", "Crew onsite at:"]; //address appears twice || Maybe also get street number and street name so less prompts
		alertPrompt(blanksArray);
		var tempInput = document.createElement("textarea");
	  ...
	  case "waterOffPlanned":
	  	blanksArray = ["Name?", "Planned Water Off Location?", "ETR?"];
		alertPrompt(blanksArray);
		var tempInput = document.createElement("textarea");
	  ...
	  case "New Scada Alarm":
		...
		alertPrompt(blanksArray);
		var tempInput = document.createElement("textarea");
        	tempInput.value = "<strong><font color='#ff0000'>insert alarms here</font></strong><p>PR issued</p>";
        	document.body.appendChild(tempInput);
        	tempInput.select();
        	document.execCommand("copy");
        	document.body.removeChild(tempInput);
